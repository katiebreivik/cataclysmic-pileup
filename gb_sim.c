...
int main(int argc, char *argv[])
{
    ...
    for(int n=0; n<N; n++)
    {
        ...
        /* generate waveform */
        int BW = 2*galactic_binary_bandwidth(orbit->L, orbit->fstar, f0, dfdt, costheta, amp, T, NFREQ); //bandwidth of waveform
        double *Atemp = malloc(sizeof(double)*2*BW);
        double *Etemp = malloc(sizeof(double)*2*BW);
        double *Xtemp = malloc(sizeof(double)*2*BW);
        galactic_binary(orbit, "sangria", T, 0.0, params, 8, Xtemp, Atemp, Etemp, BW, 2);
    ...
    FILE *fptr = fopen("injection_file.dat","w");
    for(int i = (int)(fstart*T); i <= (int)(fstop*T); i++)
    {
        fprintf(fptr,"%.12g %.12g %.12g %.12g %.12g\n", (double)i/T, A[2*i], A[2*i+1], E[2*i], E[2*i+1]);
    }
